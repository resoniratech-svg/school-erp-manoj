...
export default function RouteDetailPage() {
    ...
    const { data: route, isLoading, isError, refetch } = useQuery(() => transportClient.routes.get(id));

    useEffect(() => { if (route) setFormData({ name: route.name, code: route.code ?? '', description: route.description ?? '' }); }, [route]);

    const updateMutation = useMutation(() => transportClient.routes.update(id, formData), {
        onSuccess: () => { toast.success('Updated'); setIsEditing(false); refetch(); },
        onError: (e) => toast.error(isApiError(e) ? e.message : 'Failed'),
    });

    const deleteMutation = useMutation(() => transportClient.routes.delete(id), {
